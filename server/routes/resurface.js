...
// =============================================
// RESURFACING ENGINE
// Time-based recall + relevance boost
// =============================================
    ...
    for (const bucket of buckets) {
      const items = await Item.aggregate([
        {
          $match: {
            createdAt: {
              $gte: new Date(now - bucket.maxDays * DAY),
              $lte: new Date(now - bucket.minDays * DAY)
            }
          }
        },
        { $sample: { size: 2 } },
      ...
      items.forEach(item => {
        ...
        resurfaced.push({
          ...item,
          daysAgo,
          resurfaceReason: bucket.label
        });
      });
    }

    // Relevance boost: find items similar to the most recently saved
    const mostRecent = await Item.findOne().sort({ createdAt: -1 });
    if (mostRecent?.embedding?.length) {
      const olderItems = await Item.find({
        _id: { $ne: mostRecent._id },
      ...
      const similar = olderItems
        .map(item => ({
          item,
          score: cosineSimilarity(mostRecent.embedding, item.embedding)
        }))
      ...
      similar.forEach(({ item, score }) => {
        ...
        resurfaced.push({
          ...obj,
          daysAgo,
          resurfaceReason: `relevant to recent save (${Math.round(score * 100)}% similar)`,
          relevanceScore: Math.round(score * 100)
        });
      });
    }

    // If no time-based items found, fallback to random
